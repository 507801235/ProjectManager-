...
//        getData();
        ...
        if(isManager){
            Request.clientGet("task?asManager=yes&claimState=3&S=0&project_uid=" + project_uid , new NetCallBack() {
                @Override
                public void onMySuccess(JSONObject result) {
                    JSONArray list = result.getJSONArray("list");
                    String listString = JSONObject.toJSONString(list);
                    List<TaskModel> tasks = JSONObject.parseArray(listString, TaskModel.class);//把字符串转换成集合
                    TaskAdapter taskAdapter = new TaskAdapter(ProjectTaskActivity.this, R.layout.task_item, tasks);
                    task_view.setAdapter(taskAdapter);
                    task_view.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                        @Override
                        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                            TaskModel task = tasks.get(position);
                            Intent intent = new Intent(ProjectTaskActivity.this, TaskDetailActivity.class);
            ...
            Request.clientGet("task?asMember=yes&claimState=3&S=0&project_uid=" + project_uid , new NetCallBack() {
                @Override
                public void onMySuccess(JSONObject result) {
    ...
    private void getData() {
        projectTaskViewModel.getTaskList().observe(ProjectTaskActivity.this, new Observer<List<TaskModel>>() {
            @Override
